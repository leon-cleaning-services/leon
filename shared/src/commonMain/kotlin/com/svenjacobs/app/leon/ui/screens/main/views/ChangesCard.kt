...
import com.svenjacobs.app.leon.shared.resources.changes_title
    ...
    ElevatedCard(modifier = modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(vertical = 16.dp)) {
            Text(
                ...
                text = stringResource(Res.string.changes_title),
                style = MaterialTheme.typography.labelMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )

            // Not wrapped in `remember`: `displayName()` is itself `@Composable` (it calls
            // `stringResource`), so it must be invoked directly in composition.
