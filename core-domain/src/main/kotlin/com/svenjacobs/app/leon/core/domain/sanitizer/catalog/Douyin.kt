...
val DouyinSanitizer =
    Sanitizer(
        id = SanitizerId("douyin"),
        name = "Douyin",
        rules =
            persistentListOf(
                Rule.RewritePath("(?i)(.*?)(?:/Ipd:|/lpD:|/9@4\\.com|/\\d+/\\d+).*", "$1"),
                Rule.RemoveParameters(".*"),
            ),
        match =
            persistentListOf(
                Match(HostMatch.Domain("douyin.com")),
                Match(HostMatch.Domain("v.douyin.com")),
                Match(HostMatch.Domain("iesdouyin.com")),
            ),
    )
