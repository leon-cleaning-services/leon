...
val CarGurusSanitizer =
    Sanitizer(
        id = SanitizerId("cargurus"),
        name = "CarGurus",
        rules =
            persistentListOf(
                Rule.RemoveParameters(
                    "(listingId|entitySelectingHelper\\.selectedEntity)",
                    negate = true,
                )
            ),
        match = persistentListOf(Match(HostMatch.Domain("cargurus.co.uk"), pathPrefix = "/")),
    )
