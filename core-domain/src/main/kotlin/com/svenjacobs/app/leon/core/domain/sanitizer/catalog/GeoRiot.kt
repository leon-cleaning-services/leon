...
val GeoRiotSanitizer =
    Sanitizer(
        id = SanitizerId("georiot"),
        name = "GeoRiot",
        rules =
            persistentListOf(
                Rule.Follow(Source.Parameter("GR_URL"), persistentListOf(Decode.PercentDecode))
            ),
        match =
            persistentListOf(
                Match(HostMatch.Pattern("target\\.georiot\\.[^/?#:]+"), pathPrefix = "/Proxy.ashx")
            ),
    )
